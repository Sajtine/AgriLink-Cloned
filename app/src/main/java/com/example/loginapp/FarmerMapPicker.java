...
public class FarmerMapPicker extends AppCompatActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_farmer_map_picker);
    ...
    public void onMapReady(@NonNull GoogleMap googleMap){
        ...
        gmap.setOnCameraIdleListener(() -> {
            ...
            Geocoder geocoder = new Geocoder(FarmerMapPicker.this, Locale.getDefault());
            try {
                List<Address> addresses = geocoder.getFromLocation(lat, lng, 1);
                if (addresses != null && !addresses.isEmpty()) {
                    Address address = addresses.get(0);
                    String streetName = address.getThoroughfare(); // Street name
                    String barangay = address.getSubLocality(); // Barangay
    ...
    private void showUserLocation() {
        if (ActivityCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION)
                ...
                ActivityCompat.checkSelfPermission(this, Manifest.permission.ACCESS_COARSE_LOCATION)
                        != PackageManager.PERMISSION_GRANTED) {
            // Permission is not granted, so don't do anything here.
            return;
        }

        gmap.setMyLocationEnabled(true); // Safe to call now
